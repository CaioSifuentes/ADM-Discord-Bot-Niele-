...
class RecompensaT20(commands.Cog):
    ...
    async def recompensa_t20(self, interaction: discord.Interaction, nd: int, porcentagem: int):
        d_recompensa_t20 = {
            1: [250, 50],
            2: [500, 75],
            3: [750, 100],
        ...
        await interaction.response.send_message(f'```(ND{nd}+{porcentagem}%/{tibares_total}XP{experiencia_total}T$)```')
